...
export abstract class Link extends Entity {
  ...
  async close(options?: LinkCloseOptions): Promise<void> {
    ...
    const closePromise = new Promise<void>((resolve, reject) => {
      log.error(
        "[%s] The %s '%s' on amqp session '%s' is open ? -> %s",
        this.connection.id,
        this.type,
        this.name,
        this.session.id,
      ...
      if (this.isOpen()) {
        ...
        const removeListeners = () => {
          ...
          this._link.connection.removeListener(
        ...
        const onClose = (context: RheaEventContext) => {
          removeListeners();
          log[this.type](
            "[%s] Resolving the promise as the %s '%s' on amqp session '%s' " +
              "has been closed.",
            this.connection.id,
            this.type,
            this.name,
        ...
        const onError = (context: RheaEventContext) => {
          ...
          log.error(
            "[%s] Error occurred while closing %s '%s' on amqp session '%s': %O.",
            this.connection.id,
            this.type,
            this.name,
            this.session.id,
        ...
        const onDisconnected = (context: RheaEventContext) => {
          ...
          const error =
            context.connection && context.connection.error
              ...
              : context.error;
          log.error(
            "[%s] Connection got disconnected while closing amqp %s '%s' on amqp " +
              "session '%s': %O.",
            this.connection.id,
            this.type,
            this.name,
        ...
        const actionAfterTimeout = () => {
          removeListeners();
          const msg: string =
            `Unable to close the ${this.type} '${this.name}' ` +
    ...
    if (options.closeSession) {
      log[this.type](
        "[%s] %s '%s' has been closed, now closing it's amqp session '%s'.",
        this.connection.id,
        this.type,
        this.name,
        this.session.id,
  ...
  private _initializeEventListeners(): void {
    const events =
      this.type === LinkType.sender ? SenderEvents : ReceiverEvents;
    for (const eventName of Object.keys(events) as Array<keyof typeof events>) {
      this._link.on(events[eventName], (context: RheaEventContext) => {
    ...
    if (typeof this._link.eventNames === "function") {
      log.eventHandler(
        "[%s] rhea-promise '%s' object is listening for events: %o " +
          "emitted by rhea's '%s' object.",
        this.connection.id,
        this.type,
        this._link.eventNames(),
        this.type,
      );
    }
    if (typeof this._link.listenerCount === "function") {
      log.eventHandler(
        "[%s] ListenerCount for event '%s_error' on rhea's '%s' object is: %d.",
        this.connection.id,
        this.type,
        this.type,
        this._link.listenerCount(`${this.type}_error`),
      );
    }
  }
}
